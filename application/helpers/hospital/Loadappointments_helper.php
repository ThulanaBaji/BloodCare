    ...
    function loadAppointments($appointments){
        ...
        foreach ($appointments as $row) {
            ...
            if($row['status'] == APPOINTMENT_RESERVED)
                $idarray = $idarray.strval($row['id']).',';

            if(array_key_exists($date, $byDates)){
                $start = date('H:i', substr($row['starttime'], 0, -3));
                $end = date('H:i', substr($row['endtime'], 0, -3));
                $time = $start.'-'.$end;

                loadAppointment($row['status'], $time, $row['profile'], $row['name'], $row['id'], $row['message']);
            }
            else{
                if(count($byDates) == 0){
                    ...
                    echo 
                    <<<BC
                    <div class="p-5 pl-8 mb-4 border border-gray-100 rounded-lg bg-gray-50">
        ...
        if($status == APPOINTMENT_RESERVED){
            ...
            <li class="mb-10 ms-4">
                <div class="absolute -start-[22px] translate-y-4 w-11 h-11 ring-1 ring-gray-400 border-gray-50 border-2 flex items-center rounded-full overflow-hidden">
                    <img class="w-14 object-cover object-center" src="{$url}uploads/donor/profileimages/$profile">
                </div>
                <div class="transition-shadow rounded-lg max-w-xl p-4 border border-gray-400 group hover:shadow-lg">
                    <div class="flex justify-between items-center">
                        <time class="mb-1 text-sm leading-none text-gray-600 font-semibold inline-block">$time</time>
                        <p class="mb-1 ml-11 text-xs leading-none p-1.5 rounded bg-gray-200 text-gray-600 font-bold inline-block uppercase">reserved</p>
                    </div>
                    ...
                    <span class="hidden group-hover:flex flex-col sm:items-center sm:flex-row mt-3 p-3 w-full rounded bg-gray-100  gap-2">
                        ...
                        <button onclick="rejectAppointment(this)" class="flex-auto w-fit text-md font-semibold p-2 text-gray-500 hover:bg-gray-200 rounded" data-id="$id">reject</button>
                    </span>
                </div>
            </li>
BC;
        }
        else if($status == APPOINTMENT_REJECTED){
            echo 
            <<<BC
            <li class="mb-10 ms-4">
                ...
                <div class="rounded-lg max-w-xl p-4 border border-red-400">
                    <div class="flex justify-between items-center">
                        <time class="mb-1 text-sm leading-none text-gray-600 font-semibold inline-block">$time</time>
                        <p class="mb-1 ml-11 text-xs leading-none p-1.5 rounded bg-red-200 text-red-600 font-bold inline-block uppercase">rejected</p>
                    </div>
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">$name</h3>
                    <span class="mt-3 p-3 w-fit rounded bg-gray-100 flex flex-col sm:flex-row sm:items-center gap-2">
...
BC;
        ...
        else if($status == APPOINTMENT_CANCELLED){
            echo 
            <<<BC
            <li class="mb-10 ms-4">
                <div class="absolute -start-[22px] translate-y-4 w-11 h-11 ring-1 ring-red-400 border-gray-50 border-2 flex items-center rounded-full overflow-hidden">
                ...
                <div class="rounded-lg max-w-xl p-4 border border-red-400">
                    <div class="flex justify-between items-center">
                        <time class="mb-1 text-sm leading-none text-gray-600 font-semibold inline-block">$time</time>
                        <p class="mb-1 ml-11 text-xs leading-none p-1.5 rounded bg-red-200 text-red-600 font-bold inline-block uppercase">cancelled</p>
                    </div>
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">$name</h3>
                    <span class="mt-3 p-3 w-fit rounded bg-gray-100 flex flex-col sm:flex-row sm:items-center gap-2">
                        <svg class="flex-shrink-0 w-4 h-4 text-gray-600" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 18" fill="currentColor">

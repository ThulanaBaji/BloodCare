...
(function (w) {
	...
	function y(t, c) {
		...
		a = t.getContext("2d");
		c.background &&
			((a.fillStyle = c.background), a.fillRect(c.left, c.top, c.size, c.size));
		e = d.O;
		h = c.size / e;
		a.beginPath();
		for (r = 0; r < e; r += 1)
	...
	(function () {
		...
		function C(c, a) {
			...
			function b(b, a) {
				...
				if (7 <= c) {
					...
					for (k = 0; 18 > k; k += 1)
						(m = !b && 1 == ((f >> k) & 1)),
							(r[Math.floor(k / 3)][(k % 3) + d - 8 - 3] = m);
					for (k = 0; 18 > k; k += 1)
						(m = !b && 1 == ((f >> k) & 1)),
							(r[(k % 3) + d - 8 - 3][Math.floor(k / 3)] = m);
				}
				if (null == g) {
					b = t.I(c, h);
					f = B();
					...
					for (n = 0; n < b.length; n += 1) {
						...
						for (l = 0; l < q[n].length; l += 1)
							(w = l + v.b() - q[n].length), (q[n][l] = 0 <= w ? v.c(w) : 0);
					}
					for (l = f = 0; l < b.length; l += 1) f += b[l].o;
					f = Array(f);
					for (l = u = 0; l < k; l += 1)
						for (n = 0; n < b.length; n += 1)
							l < p[n].length && ((f[u] = p[n][l]), (u += 1));
					for (l = 0; l < m; l += 1)
						for (n = 0; n < b.length; n += 1)
							l < q[n].length && ((f[u] = q[n][l]), (u += 1));
					g = f;
				}
				b = g;
				f = -1;
		...
		function z(c, a) {
			...
			var e = (function () {
					...
					for (var r = Array(c.length - b + a), d = 0; d < c.length - b; d += 1)
						r[d] = c[d + b];
					return r;
				})(),
				b = {
					...
					multiply: function (a) {
						for (var h = Array(b.b() + a.b() - 1), c = 0; c < b.b(); c += 1)
							for (var g = 0; g < a.b(); g += 1)
								h[c + g] ^= v.i(v.g(b.c(c)) + v.g(a.c(g)));
						return z(h, 0);
					},
					l: function (a) {
						if (0 > b.b() - a.b()) return b;
						for (
							var c = v.g(b.c(0)) - v.g(a.c(0)), h = Array(b.b()), g = 0;
							g < b.b();
							g += 1
						)
							h[g] = b.c(g);
		...
		var A = { L: 1, M: 0, Q: 3, H: 2 },
			y = (function () {
				...
				var a = [
					...
					e = {
						w: function (b) {
							for (var a = b << 10; 0 <= c(a) - c(1335); )
								a ^= 1335 << (c(a) - c(1335));
							return ((b << 10) | a) ^ 21522;
						},
						A: function (b) {
							for (var a = b << 12; 0 <= c(a) - c(7973); )
								a ^= 7973 << (c(a) - c(7973));
							return (b << 12) | a;
						},
						G: function (b) {
						...
						C: function (b) {
							for (var a = z([1], 0), c = 0; c < b; c += 1)
								a = a.multiply(z([1, v.i(c)], 0));
							return a;
						},
						f: function (b, a) {
							if (4 != b || 1 > a || 40 < a)
								throw Error("mode: " + b + "; type: " + a);
							return 10 > a ? 8 : 16;
						},
						D: function (b) {
					...
					e = {
						I: function (b, a) {
							...
							if ("undefined" == typeof e)
								throw Error(
									"bad rs block @ typeNumber:" + b + "/errorCorrectLevel:" + a
								);
							b = e.length / 3;
							a = [];
							for (var d = 0; d < b; d += 1)
								for (
									var g = e[3 * d], h = e[3 * d + 1], t = e[3 * d + 2], p = 0;
									p < g;
									p += 1
								) {
									var q = t,

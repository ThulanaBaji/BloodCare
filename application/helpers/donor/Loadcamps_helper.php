...
if (!function_exists('loadCamps')) {
    function loadCamps($camps)
    {
        foreach ($camps as $row) {
            ...
            $cancelmessage = $row['message'];

            $row['status'] = $maxseats == $curseats ? CAMP_FILLED : $row['status'];

            $border = $row['status'] == CAMP_FILLED ? 'border-green-400' : (($row['status'] == CAMP_CANCELLED || $row['status'] == CAMP_REVOKED ) ? 'border-red-400' : '');
            $label = $row['status'] == CAMP_FILLED ?
                ...
                :(
                    ($row['status'] == CAMP_CANCELLED || $row['status'] == CAMP_REVOKED )  ?
            ...
            $footer = $row['status'] == CAMP_FILLED ?
                ...
                : ( ($row['status'] == CAMP_CANCELLED || $row['status'] == CAMP_REVOKED ) ? 
                    ...
                    <code class="ml-1 text-sm font-bold">'.$cancelmessage.'</code>
                </span>'
                :

...
if (!function_exists('loadJoinedCamps')) {
    function loadJoinedCamps($camps)
    {
        foreach ($camps as $row) {
            ...
            $footer = $row['status'] == CAMP_CANCELLED ?
                ...
                '<div class="flex gap-1 pb-2 flex-wrap justify-end py-1 pt-4">
                    ...
                    <button onclick="quitCamp(this)" data-id='.$id.' class="w-20 cursor-pointer text-md font-semibold p-2 text-gray-500 hover:text-gray-800 gap-1 flex items-center justify-center hover:bg-gray-200 max-w-xl rounded">
                        quit
                    </button>
                </div>';
